...
def leastsquare(functype,pinit,xdata,ydata,yerr,logfile,verbose):
	...
	coeffs = []
	covar = []
	if (functype == 'poly0' or functype == 'poly1con' or functype == 'sineCompareBinPSF'):
		coeffs.append(out[0])
		covar.append(out[1])
	else:
		coeffs = out[0]
...
def lsqclip(functype,pinit,x,y,yerr,rej_lo,rej_hi,niter,logfile,verbose):
	...
	if (len(x) == 0): status = kepmsg.exit('ERROR -- KEPFIT.LSQCLIP: x data array is empty')
	if (len(y) == 0): status = kepmsg.exit('ERROR -- KEPFIT.LSQCLIP: y data array is empty')
	if (len(x) < len(pinit)): kepmsg.warn(logfile,'WARNING -- KEPFIT.LSQCLIP: no degrees of freedom')
	...
	for ix in range(npts[iiter]):
		if (y[ix] - fit[ix] < rej_hi * sigma and fit[ix] - y[ix] < rej_lo * sigma):
			tmpx.append(x[ix])
			tmpy.append(y[ix])
		if (yerr != None): 
